...
function signPaapi(params: PaapiParams, region: string, host: string, accessKey: string, secretKey: string) {
  // Placeholder for V4 signing (omitted for brevity). We'll implement when keys are available.
  // Return { url, headers } for fetch.
  return { url: `https://${host}/paapi5/searchitems`, headers: {} as Record<string, string> }
}

export function buildPaapiRequest(keyword: string, region = 'us-east-1', host = 'webservices.amazon.com') {
  ...
  if (!accessKey || !secretKey || !partnerTag) throw new Error('Missing PA-API credentials')
