    ...
    <main className="min-h-screen bg-gradient-to-br from-slate-900 via-purple-900 to-slate-900 relative overflow-hidden">
      ...
      <div className="container mx-auto px-4 py-12 space-y-8 relative z-10">
              ...
              <div className="grid grid-cols-2 gap-6">
                <div className="text-center">
                  ...
                  <div className="text-2xl font-bold text-green-400">
                    {stats.vendor.subscriptionStatus}
                  </div>
                </div>
                <div className="text-center">
              ...
              {stats && stats.products.length > 0 ? (
                <div className="space-y-3">
                  {stats.products.map((p) => (
                    <div
                      key={p.id}
                      className="flex items-center justify-between p-3 bg-white/5 rounded-lg border border-white/10 gap-3"
                    >
        ...
        <Card className="backdrop-blur-lg bg-white/10 border-white/20 shadow-2xl">
          <CardHeader>
            <CardTitle className="text-white flex items-center gap-2">
              🛡️ Moderation & Ingestion
            </CardTitle>
          </CardHeader>
          <CardContent>
            <div className="text-purple-200 mb-4">
              Paste product URLs (one per line) to ingest without CSV.
            </div>
            <BulkUrlIngestForm />
          </CardContent>
        </Card>
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (res.ok) {
        ...
        setMessage({
          type: 'error',
          text: error.message || 'Failed to submit product',
        })
      }
    } catch (error) {
  ...
  return (
    <div className="grid lg:grid-cols-2 gap-6">
      <Card className="backdrop-blur-lg bg-white/10 border-white/20 shadow-2xl">
        <CardHeader>
          <CardTitle className="text-white flex items-center gap-2">
            Insights (last {data?.metricsWindowDays ?? 7} days)
          </CardTitle>
        </CardHeader>
        <CardContent>
          ...
          {!loading && data && (
            <div className="grid grid-cols-2 gap-4 text-sm text-purple-200">
              ...
              <MetricTile
                label="CTR"
                value={`${((data.rates?.ctr ?? 0) * 100).toFixed(2)}%`}
              />
              <MetricTile
                label="Save Rate"
                value={`${((data.rates?.saveRate ?? 0) * 100).toFixed(2)}%`}
              />
          ...
          <CardContent className="text-sm text-purple-200 space-y-3">
            <div className="flex items-center justify-between">
              ...
              <span className="text-white font-medium uppercase">
                {data.vendor?.plan || 'BASIC'}
              </span>
            </div>
            <div className="flex items-center justify-between">
              <span>Subscription Status</span>
              <span className="text-white font-medium">
                {data.vendor?.subscriptionStatus || 'INACTIVE'}
              </span>
            </div>
            <div className="flex items-center justify-between">
              <span>Current Period Ends</span>
              <span className="text-white font-medium">
                {data.vendor?.currentPeriodEnd
                  ? new Date(data.vendor.currentPeriodEnd).toLocaleDateString()
                  : '—'}
              </span>
            </div>
          </CardContent>
        ...
        <div className="space-y-2">
          {items.map((item) => (
            <div
              ...
              className="flex items-center justify-between bg-white/5 px-3 py-2 rounded border border-white/10 text-sm"
            >
