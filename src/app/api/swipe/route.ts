...
export async function POST(request: NextRequest) {
  try {
    ...
    if (!existingProduct && incoming && incoming.affiliateUrl) {
      try {
        ...
        const found = await prisma.product.findFirst({
          where: { affiliateUrl: url.toString() },
        })
        if (found) {
    ...
    if (action === 'RIGHT' || action === 'SAVED') {
      ...
      if (product && product.embedding) {
        ...
        if (user.embedding && user.embedding.length > 0) {
          ...
          const newEmbedding = user.embedding.map(
    ...
    return NextResponse.json({
      ...
      savedCount,
      message:
        action === 'SAVED' ? 'Product saved to your list!' : 'Swipe recorded',
    })
  } catch (error) {
