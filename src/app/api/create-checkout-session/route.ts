...
export async function POST(request: NextRequest) {
  try {
    ...
    const md = {
      vendorEmail: String(vendorEmail || productData?.vendorEmail || '').slice(
        0,
        200
      ),
      ...
      image0:
        Array.isArray(productData?.images) && productData.images[0]
          ? String(productData.images[0]).slice(0, 500)
          : '',
    }
      ...
      line_items: [
        {
          price_data: {
            ...
            unit_amount: Math.round(
              Number(process.env.NEXT_PUBLIC_BASIC_PRICE_USD || '9') * 100
            ),
          },
          quantity: 1,
        },
