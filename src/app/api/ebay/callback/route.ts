...
const CALLBACK_ENDPOINT =
  process.env.EBAY_CALLBACK_URL ||
  'https://presentgogo.vercel.app/api/ebay/callback'
...
export async function GET(request: NextRequest) {
  ...
  if (challengeCode) {
    if (!VERIFICATION_TOKEN) {
      console.error(
        '[eBay OAuth] Missing EBAY_VERIFICATION_TOKEN env var. Cannot respond to challenge.'
      )
  ...
  return NextResponse.json({
    success: true,
    message:
      'eBay OAuth callback received. Complete the token exchange on the server.',
  })
}
