  ...
  try {
    ...
    const payload: ProductData = {
      ...
      vendorEmail: productData.vendorEmail
        ? String(productData.vendorEmail)
        : user.email || '',
    }
    ...
    if (!openAiKey) {
      ...
      return NextResponse.json(
        { error: 'Server misconfigured: OpenAI unavailable' },
        { status: 500 }
      )
    }
      ...
      input: [
        {
          ...
          content: [
            {
              type: 'input_text',
              text: 'You are a product categorization expert. Always respond with valid JSON.',
            },
          ],
        },
        {
    ...
    return NextResponse.json({
      ...
      message:
        'Product submitted successfully. It will be reviewed before appearing in recommendations.',
    })
  } catch (error) {
