...
export async function GET(req: NextRequest) {
  try {
    ...
    const actionCounts = actionGroups.reduce<Record<string, number>>(
      (acc, group) => {
    ...
    for (const usage of productUsage) {
      ...
      vendorCounts.set(
        product.vendorEmail,
        (vendorCounts.get(product.vendorEmail) || 0) + usage._count._all
      )
    }
    ...
    const rates = {
      ...
      engagementRate:
        impressions > 0 ? (clicks + saves + likes) / impressions : 0,
    }
