...
export async function GET(req: NextRequest) {
  try {
    ...
    const perProduct: Record<
      string,
      { impressions: number; clicks: number; saves: number; likes: number }
    > = {}

    for (const group of eventGroups) {
      if (!group.productId) continue
      const current = perProduct[group.productId] || {
        impressions: 0,
        clicks: 0,
        saves: 0,
        likes: 0,
    ...
    const productMetrics = products.map((product) => {
      const metrics = perProduct[product.id] || {
        impressions: 0,
        clicks: 0,
        saves: 0,
        likes: 0,
      ...
      const saveRate =
        metrics.impressions > 0 ? metrics.saves / metrics.impressions : 0
      return {
        id: product.id,
        title: product.title,
