...
import { buildGeoInfo, COUNTRY_TO_AMAZON } from '@/src/shared/constants/geo'
...
export class RainforestProvider extends BaseProvider {
   ...
   */
  private mapProductDetailToProduct(
    item: RainforestProductDetail
  ): BaseProduct | null {
    const title = item.title || ''
    const url = item.link || ''
    ...
    if (item.specifications) {
      for (const spec of item.specifications) {
        if (spec.name.toLowerCase().includes('weight')) {
          const weightMatch = spec.value.match(
            /(\d+\.?\d*)\s*(lb|lbs|pound|pounds|oz|ounces|kg|kilograms)/i
          )
          if (weightMatch) {
    ...
    const bestSeller =
      ...
      item.bestsellers_rank.length > 0 &&
      item.bestsellers_rank[0].rank <= 100
...
export async function syncRainforestByKeyword(
  ...
  if (!apiKey) {
    ...
    return {
      ...
      skipped: 0,
      errors: 1,
      errorMessages: ['missing RAINFOREST_API_KEY'],
      products: [],
      success: false,
      duration: 0,
    }
