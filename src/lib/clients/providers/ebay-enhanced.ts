...
export class EbayProvider extends BaseProvider {
  ...
  constructor(
    appIdOrConfig?:
      | string
      | { clientId?: string; clientSecret?: string; timeout?: number },
    oauthToken?: string,
    campaignId?: string,
    config: ProviderConfig = {}
  ) {
    super()
    if (typeof appIdOrConfig === 'string' || !appIdOrConfig) {
      this.appId = appIdOrConfig as string | undefined
      ...
      this.appId = appIdOrConfig.clientId
      this.oauthToken = ''
      this.campaignId = undefined
      this.config = {
        rateLimit: 1000,
        retryAttempts: 3,
        timeout: appIdOrConfig.timeout ?? 30000,
        ...config,
      }
    }
  }
  ...
  async getAppToken(): Promise<
    { accessToken: string; tokenType: 'Bearer'; expiresAt: string } | undefined
  > {
    if (this.tokenCache) {
      const exp = new Date(this.tokenCache.expiresAt).getTime()
      if (Date.now() < exp - 120000) return this.tokenCache
      // expired instance cache → clear
      this.tokenCache = undefined
    }
    try {
      const accessToken = await this.ensureTokenString()
  ...
  private async ensureTokenString(): Promise<string> {
    // Always fetch a fresh token when no valid instance cache exists
  ...
  private mapSearchResultToProduct(item: EbaySearchResult): BaseProduct | null {
    ...
    const shippingCost = primaryShipping?.shippingCost?.value
      ...
      : undefined
    const freeShipping =
      shippingCost === 0 || primaryShipping?.shippingCost?.value === '0.00'
    ...
    if (
      primaryShipping?.minEstimatedDeliveryDate &&
      primaryShipping?.maxEstimatedDeliveryDate
    ) {
      ...
      deliveryMin = Math.ceil(
        (minDate.getTime() - now.getTime()) / (1000 * 60 * 60 * 24)
      )
      deliveryMax = Math.ceil(
        (maxDate.getTime() - now.getTime()) / (1000 * 60 * 60 * 24)
      )
    ...
    if (conditionStr.includes('NEW')) {
      ...
      conditionStr.includes('LIKE NEW') ||
      conditionStr.includes('LIKE_NEW')
    ) {
      condition = ProductCondition.LIKE_NEW
    } else if (
      conditionStr.includes('VERY GOOD') ||
      ...
      conditionStr.includes('ACCEPTABLE') ||
      conditionStr.includes('FAIR')
    ) {
      condition = ProductCondition.USED_ACCEPTABLE
    } else if (
      conditionStr.includes('REFURBISHED') ||
      conditionStr.includes('CERTIFIED')
    ) {
  ...
  private calculateQualityScore(factors: {
    ...
    if (factors.hasShippingInfo) score += 0.1
    if (factors.hasDeliveryInfo) score += 0.1
    if (factors.hasRating) score += 0.1
    if (factors.hasStock) score += 0.15
    if (factors.hasSeller) score += 0.1

    return Math.min(score, 1.0)
  }

  private resolveMarketplaceId(
  ...
  private async fetchWithRetry(
    ...
    try {
      ...
      const tokenObj = await this.getAppToken()
      ...
      if (!response.ok) {
        if (response.status === 401) {
          // clear instance cache on 401 and retry
          this.tokenCache = undefined
        }
        throw new Error(
...
export async function syncEbayByKeyword(
  ...
  if (!clientId || !oauthToken) {
    ...
    return {
      ...
      skipped: 0,
      errors: 1,
      errorMessages: ['missing ebay credentials'],
      products: [],
      success: false,
      duration: 0,
    }
