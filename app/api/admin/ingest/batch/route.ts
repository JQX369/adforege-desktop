...
export async function POST(req: NextRequest) {
  try {
    ...
    for (const raw of items) {
      try {
        // Basic validation
        if (!raw.title || !raw.description || !raw.price || !raw.url || !raw.imageUrl) {
        ...
        try {
          const embeddingResp = await openai.embeddings.create({ model: 'text-embedding-3-small', input: embeddingText })
        ...
        const domain = (raw.merchantDomain || new URL(urlCanonical).hostname).replace(/^www\./, '')
        let merchant = await prisma.merchant.findUnique({ where: { domain } })
        if (!merchant) {
          merchant = await prisma.merchant.create({ data: { name: raw.retailer || domain, domain, affiliateProgram: raw.affiliateProgram || null } })
        }
        merchantId = merchant.id

        // Upsert Product by canonical URL or ASIN
        let existing = await prisma.product.findFirst({ where: { OR: [{ urlCanonical }, { asin: raw.asin || '' }] } })
