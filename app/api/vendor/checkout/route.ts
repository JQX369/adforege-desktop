...
import { getPriceIdForTier, getCurrencyFromCountry, type SubscriptionTier, type SupportedCurrency } from '@/lib/prices'
...
export async function POST(req: NextRequest) {
	try {
		const body = (await req.json().catch(() => ({}))) as { tier?: SubscriptionTier, quantity?: number }
		const tier = body?.tier
		const quantity = Math.max(1, Math.min(100, Math.floor((body?.quantity as number) || 1)))
		...
		console.log(`Checkout: tier=${tier}, currency=${currency}, quantity=${quantity}, priceId=${priceId}`)
		if (!priceId) {
		...
		if (!customerId) {
			...
			await prisma.vendor.update({ where: { id: vendor.id }, data: { stripeCustomerId: customerId } })
		}
