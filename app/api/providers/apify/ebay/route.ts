...
export async function POST(req: NextRequest) {
  try {
    ...
    const { startUrls = [] } = await req.json().catch(() => ({ startUrls: [] })) as { startUrls: string[] }
    const apifyToken = process.env.APIFY_TOKEN || ''
    if (!apifyToken) return NextResponse.json({ error: 'Missing APIFY_TOKEN' }, { status: 500 })
    ...
    const runRes = await fetch('https://api.apify.com/v2/acts/apify~web-scraper/runs?wait=1', {
      method: 'POST',
    ...
    if (!datasetId) return NextResponse.json({ error: 'Apify run failed' }, { status: 500 })

    const itemsRes = await fetch(`https://api.apify.com/v2/datasets/${datasetId}/items`)
    const items = await itemsRes.json()
    const urls: string[] = Array.from(new Set((items || []).map((i: any) => i?.url).filter(Boolean)))

    let created = 0, updated = 0
    for (const url of urls) {
      const cleaned = cleanProductUrl(url)
      const title = cleaned.split('/').slice(-1)[0]?.replace(/[-_]/g, ' ') || 'eBay Product'
      const affiliateUrl = buildAffiliateUrl(cleaned)

      let embedding: number[] = []
      try {
        const er = await openai.embeddings.create({ model: 'text-embedding-3-small', input: title })
        embedding = er.data[0].embedding
      } catch {}

      const qualityScore = (affiliateUrl && affiliateUrl !== '#' ? 0.5 : 0) + (embedding.length ? 0.5 : 0)
      ...
      if (existing) { await prisma.product.update({ where: { id: existing.id }, data: dataUp }); updated++ } else { await prisma.product.create({ data: dataUp }); created++ }
    }
