  ...
  if (process.env.EBAY_CLIENT_ID && process.env.EBAY_CLIENT_SECRET) {
    try {
      ...
      if (token) {
        ...
        checks.ebay.tokenAge = Date.now() - (token.expiresAt ? new Date(token.expiresAt).getTime() : Date.now())
      } else {
        checks.ebay.error = 'Failed to obtain token'
      }
    } catch (error) {
      checks.ebay.error = error instanceof Error ? error.message : 'Unknown error'
    }
  } else {
