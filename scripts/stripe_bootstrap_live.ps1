...
# Always use live mode per your request
$StripeFlags = @('--live','--color','off')
function S { param([Parameter(ValueFromRemainingArguments=$true)][string[]]$Args) & $Stripe @StripeFlags @Args }

function Ensure-Product {
  param([string]$Name)
  $json = S products list --active --limit 100
  $parsed = $json | ConvertFrom-Json
  $id = $parsed.data | Where-Object { $_.name -eq $Name } | Select-Object -First 1 -ExpandProperty id
  if (-not $id) {
    $created = S products create --name $Name | ConvertFrom-Json
    $id = $created.id
  }
  return $id
}
...
function Ensure-Price {
  ...
  $json = S prices list --product $ProductId --limit 100
  $parsed = $json | ConvertFrom-Json
  ...
  $created = S prices create --product $ProductId --currency $Currency --unit-amount $Amount --recurring.interval month --lookup-key $Lookup --nickname $Lookup | ConvertFrom-Json
  return $created.id
}
...
@(
  '# Generated by PowerShell bootstrap (LIVE)',
