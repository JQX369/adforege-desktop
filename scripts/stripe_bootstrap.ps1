...
function Ensure-Product {
  param([string]$Name)
  $json = & $Stripe products list  --active true --limit 100
  $parsed = $null
  try { $parsed = $json | ConvertFrom-Json } catch {}
  $id = $null
  if ($parsed -and $parsed.data) {
    $id = $parsed.data | Where-Object { $_.name -eq $Name } | Select-Object -First 1 -ExpandProperty id
  }
  if (-not $id) {
    $created = & $Stripe products create  --name $Name | ConvertFrom-Json
    $id = $created.id
  }
  return $id
}
...
function Ensure-Price {
  ...
  $json = & $Stripe prices list  --product $ProductId --limit 100
  $parsed = $null
  try { $parsed = $json | ConvertFrom-Json } catch {}
  $id = $null
  if ($parsed -and $parsed.data) {
    $id = $parsed.data | Where-Object { $_.currency -eq $Currency -and $_.unit_amount -eq $Amount -and $_.recurring.interval -eq 'month' -and $_.active -eq $true } | Select-Object -First 1 -ExpandProperty id
  }
  if (-not $id) {
    $created = & $Stripe prices create  --product $ProductId --currency $Currency --unit-amount $Amount --recurring.interval month --lookup-key $Lookup --nickname $Lookup | ConvertFrom-Json
    $id = $created.id
  }
  return $id
}
...
@(
  "# Generated by PowerShell bootstrap",
