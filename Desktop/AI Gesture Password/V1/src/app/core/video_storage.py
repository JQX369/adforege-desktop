...
class VideoAnalysisStorage:
    ...
    CANONICAL_EMOTIONS = ("joy", "surprise", "neutral", "calm", "sadness", "love")
    EMOTION_ALIASES = {
        "delight": "joy",
        "excited": "joy",
        "happy": "joy",
    ...
    def __init__(self, storage_dir: str = "video_analyses"):
        ...
        self._db_mtime = self.db_file.stat().st_mtime if self.db_file.exists() else 0
        
    def _load_database(self) -> Dict:
        """Load database from file, with backup recovery on corruption"""
        default_db = {
            "analyses": {},
            "reactions": {},
            "reaction_jobs": {},
            ...
            try:
                with open(self.db_file, 'r') as f:
                    ...
                    for key in default_db.keys():
                        if key == "settings" and key not in data:
                            data[key] = default_db[key]
                        else:
                            data.setdefault(key, {})
                    self._db_mtime = self.db_file.stat().st_mtime
                    return data
                ...
                if backup_path.exists():
                    ...
                    try:
                        with open(backup_path, 'r') as f:
                            ...
                            for key in default_db.keys():
                                data.setdefault(key, {})
                            logger.info(f"Successfully recovered {len(data.get('analyses', {}))} analyses from backup")
                            # Restore the backup as main file
                            shutil.copy2(backup_path, self.db_file)
                            self._db_mtime = self.db_file.stat().st_mtime
                            return data
                    except Exception as backup_err:
                        logger.error(f"Failed to recover from backup: {backup_err}")
                return default_db
    ...
    def _save_database(self):
        ...
        try:
            ...
            try:
                ...
                if os.name == 'nt' and self.db_file.exists():
    ...
    def reload_database(self) -> None:
        """Force reload database from disk, discarding cached data."""
        logger.info("Reloading database from disk")
        old_count = len(self.db.get("analyses", {}))
        self.db = self._load_database()
        new_count = len(self.db.get("analyses", {}))
        logger.info(f"Database reloaded: {old_count} -> {new_count} analyses")
    
    def _check_and_reload_if_stale(self) -> bool:
        """Check if database file has been modified externally and reload if needed.
        
        Returns:
            True if database was reloaded, False otherwise.
        """
        if not self.db_file.exists():
            return False
        
        try:
            current_mtime = self.db_file.stat().st_mtime
            if current_mtime > self._db_mtime:
                logger.info(f"Database file modified externally (mtime: {current_mtime} > cached: {self._db_mtime}), reloading")
                self.reload_database()
                return True
        except OSError as e:
            logger.warning(f"Failed to check database file mtime: {e}")
    ...
    def _canonical_emotion(self, emotion: Optional[str]) -> str:
        if not emotion:
            return "neutral"
        return self.EMOTION_ALIASES.get(emotion.lower(), "neutral")

    def _normalize_score_map(self, scores: Dict[str, float], *, as_percentage: bool) -> Dict[str, float]:
        """Map arbitrary emotion keys onto canonical buckets."""
        normalized = {emotion: 0.0 for emotion in self.CANONICAL_EMOTIONS}
        total = 0.0
        for raw_emotion, value in (scores or {}).items():
            if value is None:
                continue
            canonical = self._canonical_emotion(raw_emotion)
            normalized[canonical] += float(value)
            total += float(value)

        if total > 0:
            factor = 100.0 if as_percentage else 1.0
            normalized = {emotion: (value / total) * factor for emotion, value in normalized.items()}
        else:
            normalized["neutral"] = 100.0 if as_percentage else 1.0
        return normalized

    def _normalize_timeline_entry(self, entry: Dict) -> Dict:
        """Ensure each timeline sample exposes canonical emotion fractions."""
        normalized_scores = self._normalize_score_map(entry.get("scores", {}), as_percentage=False)
        normalized_entry = {
            "timestamp": float(entry.get("timestamp", 0.0)),
            "engagement": float(entry.get("engagement", 0.0)),
            "emotion": self._canonical_emotion(entry.get("emotion")),
        }
        normalized_entry.update(normalized_scores)
    ...
    def create_analysis(
        ...
        *,
        ...
        try:
            ...
            video_dest = Path(video_path)  # Use original path
        else:
        ...
        suffix = video_dest.suffix.lower()
        playback_ready = suffix == ".mp4"
        playback_path = str(video_dest) if playback_ready else None
        ...
        analysis_entry = {
            ...
            "playback_ready": bool(playback_ready),  # Ensure it's always True or False, never None
            "playback_video_path": playback_path,
            "playback_job_id": None,
        }

        self.db["analyses"][analysis_id] = analysis_entry
    ...
    def set_playback_job_id(self, analysis_id: str, job_id: Optional[str]) -> bool:
        analysis = self.db["analyses"].get(analysis_id)
        if not analysis:
            return False
        analysis["playback_job_id"] = job_id
        self._save_database()
        return True

    def mark_playback_ready(self, analysis_id: str, playback_path: str) -> bool:
        analysis = self.db["analyses"].get(analysis_id)
        if not analysis:
            return False
        analysis["playback_ready"] = True
        analysis["playback_video_path"] = playback_path
        analysis["playback_job_id"] = None
        self._save_database()
        return True

    def generate_playback_copy(self, analysis_id: str) -> str:
        analysis = self.db["analyses"].get(analysis_id)
        if not analysis:
            raise RuntimeError(f"Analysis {analysis_id} not found")
        video_path = Path(analysis["video_path"])
        if not video_path.exists():
            raise FileNotFoundError(f"Source video missing for {analysis_id}")
        result = self._ensure_playback_copy(video_path)
        if not result:
            raise RuntimeError("Failed to create playback copy")
        self.mark_playback_ready(analysis_id, result)
        return result

    def _ensure_playback_copy(self, video_path: Path) -> Optional[str]:
        """Ensure there is an MP4 version ready for browser playback."""
        ...
        if not ffmpeg_path:
            logger.warning("FFmpeg not available; skipping playback conversion for %s", video_path)
            return None

        playback_path = video_path.with_name(f"{video_path.stem}_playback.mp4")

        if playback_path.exists() and playback_path.stat().st_mtime >= video_path.stat().st_mtime:
            return str(playback_path)

        try:
            scale_filter = "scale=w='if(gt(iw,1920),1920,iw)':h='if(gt(ih,1080),1080,ih)':force_original_aspect_ratio=decrease"
            subprocess.run(
                [
                    ffmpeg_path,
                    "-y",
                    ...
                    "23",
                    "-profile:v",
                    "main",  # Use 'main' instead of 'high' for better browser compatibility
                    "-level",
                    "4.0",
                    "-maxrate",
                    "8000k",
            ...
            logger.warning("Failed to create playback copy for %s: %s", video_path, exc)
            return None
        
    def save_reaction(self, analysis_id: str, reaction_data: Dict, reaction_id: Optional[str] = None) -> Optional[str]:
        ...
        reaction_id = reaction_id or f"reaction_{datetime.now().strftime('%Y%m%d_%H%M%S')}"
        
        # Determine dominant emotion
        raw_summary = reaction_data.get("emotion_summary", {})
        emotion_summary = self._normalize_score_map(raw_summary, as_percentage=True)
        dominant_emotion = max(emotion_summary, key=emotion_summary.get) if emotion_summary else "neutral"
        normalized_timeline = [
            self._normalize_timeline_entry(entry)
            for entry in reaction_data.get("emotion_timeline", [])
        ]
        ...
        reaction_entry = {
            ...
            "created_at": datetime.now().isoformat(),
            "emotion_timeline": normalized_timeline,
            ...
            "video_path": reaction_data.get("video_path")
        }
        ...
        analysis_reaction = {
            ...
            "video_path": reaction_data.get("video_path"),
            "emotion_summary": emotion_summary,
            "created_at": reaction_entry["created_at"],
        }
    ...
    def _update_analysis_aggregates(self, analysis_id: str):
        ...
        total_engagement = sum(r.get("engagement_score", 0.0) for r in full_reactions)
        analysis["avg_engagement"] = total_engagement / len(full_reactions)
        analysis["audience_engagement"] = analysis["avg_engagement"]
        analysis["score_source"] = "audience_only"
        
        # Combine emotion summaries from full reaction data using canonical buckets
        combined_emotions: Dict[str, List[float]] = {emotion: [] for emotion in self.CANONICAL_EMOTIONS}
        for reaction in full_reactions:
            emotion_summary = reaction.get("emotion_summary", {})
            for emotion in self.CANONICAL_EMOTIONS:
                combined_emotions[emotion].append(emotion_summary.get(emotion, 0.0))
                
        # Average emotion percentages
        analysis["emotion_summary"] = {
            emotion: (sum(values) / len(values)) if values else 0.0
            for emotion, values in combined_emotions.items()
        }
        ...
        logger.info(
            "REACTION_LOG: reaction.aggregate.updated - analysis_id=%s, reactions=%d, engagement=%.3f",
            analysis_id,
            len(full_reactions),
            analysis["audience_engagement"],
        )
    ...
    def get_analysis(self, analysis_id: str, *, check_stale: bool = True) -> Optional[Dict]:
        """Get analysis data by ID
        
        Args:
            analysis_id: The analysis ID to retrieve
            check_stale: If True, check if database file was modified externally
                        and reload if needed before returning data.
        ...
        analysis = self.db["analyses"].get(analysis_id)
        if analysis:
            # Normalize legacy None values to False for backward compatibility
            # Return a copy to avoid mutating the cached dict
            analysis = dict(analysis)
            if analysis.get("playback_ready") is None:
                # Infer from file extension if not set
                video_path = analysis.get("video_path", "")
                suffix = Path(video_path).suffix.lower() if video_path else ""
                analysis["playback_ready"] = suffix == ".mp4"
        return analysis

    def get_video_path(self, analysis_id: str) -> Optional[str]:
        """Get the path to the video file for an analysis"""
        analysis = self.get_analysis(analysis_id)
        if analysis:
            return analysis.get("video_path")
        return None
        
    def get_all_analyses(self, user_id: str = None, *, check_stale: bool = True) -> List[Dict]:
    ...
    def get_reaction_summary_for_analysis(self, analysis_id: str) -> List[Dict]:
        analysis = self.db["analyses"].get(analysis_id, {})
        return analysis.get("reactions", [])
        ...
        try:
            ...
            try:
                ...
                if video_clip.audio:
                    ...
                    try:
                        from app.features.ai_breakdown.enhanced_transcript_analyzer import EnhancedTranscriptAnalyzer
    ...
    # ------------------------------------------------------------------ #
    # Reaction job metadata
    # ------------------------------------------------------------------ #
    def create_reaction_job(self, analysis_id: str, reaction_id: str, video_path: str) -> Dict:
        job = {
        ...
        self.db["reaction_jobs"][reaction_id] = job
        self._save_database()
        return job

    def update_reaction_job(self, reaction_id: str, **updates) -> Optional[Dict]:
        job = self.db["reaction_jobs"].get(reaction_id)
        if not job:
            return None
    ...
    def get_reaction_job(self, reaction_id: str) -> Optional[Dict]:
        job = self.db["reaction_jobs"].get(reaction_id)
        if job:
            job.setdefault("processing_mode", "queue")
        return job

    def list_reaction_jobs(self, analysis_id: str) -> List[Dict]:
        jobs = [
            job for job in self.db["reaction_jobs"].values()
    ...
    # ------------------------------------------------------------------ #
    # Generalized queue jobs (video uploads, reactions, etc.)
    # ------------------------------------------------------------------ #
    def create_queue_job(
        ...
        *,
        ...
        job_id = f"queue_{uuid.uuid4().hex[:12]}"
        job = {
            "job_id": job_id,
            "job_type": job_type,
            "analysis_id": analysis_id,
        ...
        self.db["queue_jobs"][job_id] = job
        self._save_database()
        return job

    def update_queue_job(self, job_id: str, **updates) -> Optional[Dict]:
        job = self.db["queue_jobs"].get(job_id)
        if not job:
            return None
        for key, value in updates.items():
            job[key] = value
    ...
    def get_queue_job(self, job_id: str) -> Optional[Dict]:
        return self.db["queue_jobs"].get(job_id)
    ...
    def list_queue_jobs(
        ...
        jobs = list(self.db["queue_jobs"].values())
        if analysis_id:
            jobs = [job for job in jobs if job.get("analysis_id") == analysis_id]
        if job_type:
            jobs = [job for job in jobs if job.get("job_type") == job_type]
        if status:
            jobs = [job for job in jobs if job.get("status") == status]
        return sorted(jobs, key=lambda job: job.get("created_at") or "", reverse=False)
    ...
    def update_settings(self, settings: Dict) -> Dict:
        ...
        self.db["settings"] = current_settings
        self._save_database()
