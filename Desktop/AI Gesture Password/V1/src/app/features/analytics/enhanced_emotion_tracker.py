...
if not MEDIAPIPE_AVAILABLE:
    logger.warning("MediaPipe not available - face mesh/detection trackers will be disabled")
if not DEEPFACE_AVAILABLE:
    logger.warning("DeepFace not available - disabling DeepFace emotion inference")

# Try to import FER in a version-tolerant way
try:
    # Newer fer releases
    ...
    try:
        ...
        logger.warning("FER library not available - FER-based emotion detection will be disabled")
...
try:
    from .emotion_optimization import ResearchBasedEmotionOptimizer, ContextAwareEmotionDetector
...
try:
    from app.features.analytics.enhanced_reaction_analyzer import EnhancedReactionAnalyzer
    ENHANCED_ANALYZER_AVAILABLE = True
except ImportError:
    ENHANCED_ANALYZER_AVAILABLE = False
    logger.warning("Enhanced reaction analyzer not available")

# Import new physiological trackers
try:
    ...
    logger.warning(f"Physiological trackers not available: {e}")
    PHYSIO_AVAILABLE = False
...
class EnhancedEmotionTracker:
    ...
    def __init__(self):
        ...
        # Initialize FER detector early to avoid first-run delay (optional backend)
        if FER is not None:
            try:
        ...
        # Initialize physiological trackers
        if PHYSIO_AVAILABLE:
    ...
    def _analyze_frame(self, frame: np.ndarray, frame_number: int) -> Optional[Dict]:
        ...
        try:
            ...
            if DEEPFACE_AVAILABLE and self.deeFace_enabled:
                try:
                ...
                logger.debug("DeepFace unavailable; skipping emotion analysis via DeepFace")
            ...
            if self.face_mesh:
                ...
                logger.debug("Skipping MediaPipe micro-expression analysis (mediapipe unavailable)")
            ...
            if self.face_detection:
                ...
                logger.debug("Skipping MediaPipe face detection confidence (mediapipe unavailable)")
            ...
            # Add physiological metrics if available
            if PHYSIO_AVAILABLE and results.multi_face_landmarks:
                landmarks = results.multi_face_landmarks[0]
                h, w, _ = frame.shape
                
                # Gaze
                gaze_data = self.gaze_tracker.process(landmarks, (h, w))
                result['gaze'] = gaze_data
                
                # Blink
                blink_data = self.blink_detector.process(landmarks, (h, w))
                result['blink'] = blink_data
                
                # Pulse
                pulse_data = self.pulse_estimator.process(frame, landmarks)
                result['pulse'] = pulse_data
                
                # Adjust engagement based on gaze and blink
                # If looking away or fatigued, reduce engagement
                if gaze_data['gaze_direction'] != 'Center':
                    result['engagement'] *= 0.7
                if blink_data['fatigue_level'] != 'Normal':
                    result['engagement'] *= 0.8
    ...
    def _complete_analysis(self):
        ...
        # Apply temporal smoothing to reduce noise in emotion readings
        smoothed_timeline = self._smooth_emotion_readings(self.emotion_history, window_size=3)

        results = {
            'analysis_id': self.current_analysis_id,
            'emotion_timeline': smoothed_timeline,
            'emotion_timeline_raw': self.emotion_history,  # Keep raw data for debugging
    ...
    def _smooth_emotion_readings(self, readings: List[Dict], window_size: int = 3) -> List[Dict]:
        """
        Apply temporal smoothing to reduce noise in emotion detection.

        Uses a sliding window to apply majority voting for emotion labels
        and averaging for numeric values like engagement and confidence.

        Args:
            readings: List of emotion reading dictionaries
            window_size: Number of readings to consider in each window (default 3)

        Returns:
            Smoothed list of emotion readings with added 'confidence' field
        """
        ...
        for i, reading in enumerate(readings):
            ...
            # Count emotions in window for majority voting
            emotion_counts = {}
            ...
            for r in window:
                ...
                total_engagement += r.get("engagement", 0.5)
                # Use existing confidence if available, otherwise estimate from emotion scores
                conf = r.get("confidence")
                if conf is None and "scores" in r:
                    scores = r.get("scores", {})
                    if scores:
                        conf = max(scores.values()) if scores.values() else 0.5
                    else:
                        conf = 0.5
                total_confidence += conf if conf is not None else 0.5

            # Use majority emotion
            consensus_emotion = max(emotion_counts, key=emotion_counts.get)
            avg_engagement = total_engagement / len(window)
            consensus_confidence = emotion_counts[consensus_emotion] / len(window)
            avg_confidence = total_confidence / len(window)

            # Create smoothed reading
            smoothed_reading = reading.copy()
            smoothed_reading["emotion"] = consensus_emotion
            smoothed_reading["engagement"] = round(avg_engagement, 3)
            smoothed_reading["confidence"] = round(avg_confidence, 3)
            smoothed_reading["consensus_score"] = round(consensus_confidence, 2)
            smoothed_reading["smoothed"] = True

            smoothed.append(smoothed_reading)
    ...
    def warm_models(self) -> bool:
        ...
        try:
            ...
            if DEEPFACE_AVAILABLE:
                ...
                logger.info("Skipping DeepFace warm-up (DeepFace not installed)")
            ...
            # 3. Warm up MediaPipe
            if self.face_mesh and self.face_detection:
                try:
                ...
                logger.info("Skipping MediaPipe warm-up (mediapipe not available)")
