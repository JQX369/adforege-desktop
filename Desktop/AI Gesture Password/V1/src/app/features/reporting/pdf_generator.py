...
class AIBreakdownLayoutConfig:
    ...
    include_yellow_highlights: bool = True
...
class PDFGenerator:
    ...
    def __init__(self):
        ...
        # Load settings
        try:
            storage = VideoAnalysisStorage(str(Path(__file__).parent.parent.parent.parent / "video_analyses"))
            self.settings = storage.get_settings()
            self.corporation_name = self.settings.get("corporation_name", "AdForege")
            self.accent_color = self.settings.get("accent_color", "#3B82F6")
        except Exception as e:
            logger.warning(f"Failed to load settings in PDFGenerator: {e}")
            self.corporation_name = "AdForege"
            self.accent_color = "#3B82F6"

        # Update global colors with accent color
        try:
            PDF_COLORS['accent'] = HexColor(self.accent_color)
        except Exception:
            PDF_COLORS['accent'] = HexColor('#3B82F6')
    ...
    def _setup_custom_styles(self):
        ...
        add_style_if_not_exists(
            ...
            fontSize=28,
            textColor=PDF_COLORS.get('accent', PDF_COLORS['text']),
            spaceAfter=16,
            spaceBefore=0,
    ...
    def _add_header_footer(self, canvas_obj, doc, video_name: str = "", report_type: str = ""):
        ...
        canvas_obj.setFillColor(PDF_COLORS.get('accent', PDF_COLORS['text_light']))
        header_text = self.corporation_name
        if report_type:
...
class ClearcastPDFGenerator(PDFGenerator):
    ...
    def generate_pdf(self, results: Dict, video_name: str = "", video_duration: float = 0.0, 
        ...
        try:
            ...
            formatted_date = report_dt.strftime("%d %b %Y")
            title_bits = [f"{self.corporation_name} Report", report_brand, formatted_date]
            ...
            # Substantiation Pack
            substantiation = results.get('substantiation_pack') or []
            if substantiation:
                story.append(PageBreak())
                story.append(Paragraph("📁 SUBSTANTIATION PACK - Required Evidence", self.styles['SectionHeader']))
                story.append(Spacer(1, self.SMALL_SPACER))
                story.append(Paragraph("Based on the claims identified in your ad, you must provide the following evidence to Clearcast:", self.styles['BodyText']))
                story.append(Spacer(1, 0.1 * inch))
                
                for item in substantiation:
                    claim_text = item.get('claim', 'Unknown Claim')
                    claim_type = item.get('type', 'General').title()
                    requirements = item.get('requirements', [])
                    
                    # Create a card for each claim
                    rows = []
                    rows.append([
                        Paragraph(f"<b>Claim:</b> &ldquo;{claim_text}&rdquo;", self.styles['CardText']),
                    ])
                    rows.append([
                        Paragraph(f"<b>Type:</b> {claim_type} Claim", self.styles['CardText']),
                    ])
                    ...
                    req_text = "<br/>".join(req_list)
                    rows.append([
                        Paragraph(f"<b>Required Evidence:</b><br/>{req_text}", self.styles['CardText']),
                    ])
                    
                    pack_table = Table(rows, colWidths=[self.content_width])
...
class AIBreakdownPDFGenerator(PDFGenerator):
    ...
    def generate_pdf(self, results: Dict, video_name: str = "", video_duration: float = 0.0,
        ...
        try:
            ...
            def get_cited_frame_image(text: str, frames: List[str]) -> Optional[Image]:
                """Extract frame citation and return Image flowable if found."""
                import re
                if not text or not frames:
                    return None
                
                # Look for [Frame X] pattern
                match = re.search(r'\[Frame\s+(\d+)\]', text, re.IGNORECASE)
                if match:
                    try:
                        frame_idx = int(match.group(1)) - 1  # 1-based to 0-based
                        if 0 <= frame_idx < len(frames):
                            frame_data = base64.b64decode(frames[frame_idx])
                            # Create image with reasonable width (e.g. 2.5 inches)
                            img_width = 2.5 * inch
                            img = Image(BytesIO(frame_data), width=img_width, height=img_width * 9/16)
                            return img
                    except Exception as e:
                        logger.warning(f"Failed to load cited frame image: {e}")
                return None
            ...
            display_name = ad_name or (video_name or "")
            story.append(Paragraph(f"{self.corporation_name} Report", self.styles['CustomTitle']))
            subtitle_lines = []
            if display_name:
                subtitle_lines.append(display_name)
            if brand_name:
            ...
            if outcome:
                ...
                if metrics:
                    ...
                    from app.features.ai_breakdown.effectiveness_benchmarks import convert_text_to_tier
            ...
            if layout.include_highlights and green_highlights:
                ...
                for i, highlight in enumerate(green_highlights, 1):
                    ...
                    if evidence:
                        ...
                        # Check for frame citation
                        frame_img = get_cited_frame_image(evidence, frames)
                        if frame_img:
                            rows.append([frame_img])
                            card_chars += 500  # Approximate size for layout calculation
                    # Impact
            ...
            if layout.include_yellow_highlights and yellow_highlights:
                story.append(Paragraph("⚠️ AREAS FOR IMPROVEMENT", self.styles['SectionHeader']))
                story.append(Spacer(1, self.SECTION_SPACER))
                for highlight in yellow_highlights[:5]:
                    rows = []
                    card_chars = 0
                    # Title
                    title = self._truncate_text(
                        highlight.get('aspect') or highlight.get('suggestion') or 'Improvement Area',
                        max_chars=100
                    )
                    card_chars += len(title)
                    rows.append([
                        self._make_paragraph(
                            f"<b>{title}</b>",
                            style_name='CardTitle',
                        )
                    ])
                    # Suggestion
                    ...
                    if highlight.get('priority'):
                        priority = str(highlight['priority']).upper()
                        rows.append([
                            self._make_paragraph(
                                f"Priority: {priority}",
                                style_name='CardText',
            ...
            if layout.include_soft_risks and soft_risks:
                ...
                for risk in soft_risks[:5]:
                    ...
                    if risk.get('evidence_text'):
                        ...
                        # Check for frame citation
                        frame_img = get_cited_frame_image(risk.get('evidence_text'), frames)
                        if frame_img:
                            paragraphs.append(frame_img)
                            card_chars += 500
            ...
            if frames:
                ...
                story.append(Spacer(1, self.SECTION_SPACER))
                story.append(Paragraph("Key frames analyzed by AI:", self.styles['BodyText']))
                story.append(Spacer(1, self.SMALL_SPACER))
                ...
                current_row = []
                row_width = 0
                max_row_width = self.content_width
                
                for i, frame_base64 in enumerate(frames):
                    try:
                        img_data = base64.b64decode(frame_base64)
                        # Target width: ~3 inches
                        target_width = 3 * inch
                        img = Image(BytesIO(img_data), width=target_width, height=target_width * 9/16)
                        
                        # Caption
                        caption = Paragraph(f"Frame {i+1}", self.styles['SmallText'])
                        
                        # Cell content
                        cell_content = [img, Spacer(1, 5), caption]
                        
                        current_row.append(cell_content)
                        
                        if len(current_row) >= 2: # 2 frames per row
                            frame_rows.append(current_row)
                            current_row = []
                    except Exception as e:
                        logger.warning(f"Failed to add frame {i} to PDF: {e}")
                
                if current_row:
                    while len(current_row) < 2:
                        current_row.append([]) # Pad with empty cells
                    frame_rows.append(current_row)
                
                if frame_rows:
                    frame_table = Table(frame_rows, colWidths=[self.content_width/2, self.content_width/2])
