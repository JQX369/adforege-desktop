...
export function ProductCard({ product, className = '', sessionId, userId }: ProductCardProps) {
  ...
  try {
    ...
    const lang = (typeof navigator !== 'undefined' && (navigator as any).language) ? (navigator as any).language : ''
  ...
  return (
    <Card className={`h-full overflow-hidden border bg-card ${className}`}>
      <div className="relative h-2/3 bg-gray-100">
        <Image
          unoptimized={Boolean(product.imageUrl && product.imageUrl.startsWith('http://'))}
      ...
      <CardContent className="p-6 h-1/3 flex flex-col justify-between">
        <div className="space-y-2">
          <h3 className="font-semibold text-lg md:text-xl line-clamp-1">{product.title}</h3>
          <p className="text-sm text-muted-foreground line-clamp-2">{product.description}</p>
          
          <div className="flex flex-wrap gap-1 mt-2">
            {product.categories.slice(0, 3).map((category) => (
              <Badge key={category} variant="outline" className="text-[10px] md:text-xs">
                {category}
              </Badge>
            ))}
          </div>
